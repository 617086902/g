...
namespace Core.Utilities
{
    public class EmailHelper
    {
        ...
        public EmailHelper(string server, string toMail, string fromMail, string subject, string emailBody,
            string displayName, string username, string password, string port, bool sslEnable, bool pwdCheckEnable)
        {
            try
            {
        ...
        public static MailSetting GetMailSetting
        {
            get
            {
                ...
                doc.Load(string.Format("{0}Mail.config", AppDomain.CurrentDomain.BaseDirectory));
        ...
        public static bool SendMail(string to, string title, string body, string attachPath)
        {
            ...
            try
            {
                body = string.Format("{0}<br/><br/><span style='color:#f00;font-size:15px;'>此邮件由系统发出，无需直接回复！</span>", body);
                EmailHelper email = new EmailHelper(GetMailSetting.Server, to, GetMailSetting.From, title, body, GetMailSetting.ShowName, GetMailSetting.From, GetMailSetting.Password, Convert.ToString(GetMailSetting.SmtpPort), false, false);
